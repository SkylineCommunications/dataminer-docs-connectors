...
This connector is exported by the [Harmonic NMX](xref:Connector_help_Harmonic_NMX) parent connector (available from range 4.1.1.x onwards). A child element using this connector is automatically created for each switch entry in the **Platform** table of the parent element, providing a dedicated view of that switch's ports, streams, services, and alarms.
...
> [!NOTE]
> It is not possible to define alarms on the tables included in this tree control, because a stream can be output for one device but also input for another device. In other words, rows of the base table can be exported to different DVEs and it is not possible to have one alarm that is linked to multiple elements.

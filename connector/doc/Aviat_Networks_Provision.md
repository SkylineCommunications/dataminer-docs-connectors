...
- **EPM card alarm linking**: All alarms generated by the EPM Manager protocol (both frontend and backend elements) have properties (System Name and System Type) that link the alarm system-wide to the EPM entity. This allows you to track alarms at the collector level for a specific EPM card. The Equipment and Normalized Alarms tables are used specifically to facilitate this linkage.
...
The **Device Name** column is used to link devices to the backend (BE) and frontend (FE) EPM elements. Make sure the **names match exactly** in both places for proper linkage.

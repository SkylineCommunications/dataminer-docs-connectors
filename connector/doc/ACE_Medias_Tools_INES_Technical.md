...
The **Lines Info** table lists all lines retrieved from the INES system. Each row represents one codec line with its live operational state.
...
> [!NOTE]
> After a Start or Stop action, the connector immediately re-polls the active-call endpoint and then refreshes the full line list, so state changes are reflected quickly without waiting for the next timer cycle.

#### Line Capability

The **Line Capability** table shows the audio codec protocol configuration for each line. This data is populated from the same `/api/rest/lines` poll that drives the Lines Info table.
...
To update a line's codec configuration, right-click a row and select **Edit Line Capability...**. This opens a form pre-filled with the current values. Once submitted, the setup is sent via `POST /api/rest/lines/:lineId/setup`, and the full line list is refreshed automatically.

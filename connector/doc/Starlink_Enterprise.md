...
| 1.0.3.x [SLC Main] | Implemented Multi-threading to poll Management Data and Telemetry Data on different threads, this will keep tables up to date with proper mapping of services and keep unconfigured terminals from persisting on tables. | Starlink Enterprise 1.0.2.10  | Element needs to be reconfigured with a new connection using the starlink api as the endpoint configured.|

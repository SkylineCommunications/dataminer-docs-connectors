...
User terminals can now be converted to dynamic virtual elements (DVEs). To generate a DVE, simply Enable the User Terminals table DVE Creation togglebutton of a specific terminal.
To remove a DVE, you are forced to execute the following steps:
1. Make sure that the User Terminals table DVE Creation togglebutton of the corresponding terminal is set to disabled.
2. Navigate to the **User Terminal DVEs Configuration** page by pressing the pagebutton on the **Configuration** page or pressing the arrow pointing down next to Configuration.
3. Press the *Delete* button of the corresponding terminal.
4. Read the warning message carefully.
5. Press *Yes* if you are sure that the DVE can be removed.

User Terminal DVEs can only be removed if the DVE Creation column contains a value other than Enabled. User terminals for which a dynamic virtual element was created won't be removed automatically if they are no longer returned by the API for more than one day and have DVE Creation set to Enabled. 
...
> [!IMPORTANT]
> Warming! Changing the Client Secret now also removes all dynamic virtual elements.
...
Every row in the **User Terminal DVEs** table represents a DVE. An empty table is completely normal. It simply means that no DVEs were generated yet. Once a DVE gets generated by enabling the User Terminals table DVE Creation togglebutton, a new row will appear in this table.

Pressing the User Terminal DVEs table Delete button is the only way to remove a DVE.
...
After creating the element, navigate to the [Configuration page](#configuration-page) to [set up the registers](#register-configuration) you want to monitor.

### Register Configuration

The Register Configuration table on the Configuration page is the heart of this connector. Each row defines a register or group of registers to poll from your Modbus device.

To add a new register to monitor:

1. Right-click anywhere in the table and select **Add new row** in the context menu.
...
> [!NOTE]
> **This connector uses 0-based register addressing**. However, device manufacturers often use different addressing conventions in their documentation:
>
...
- **Number of Registers**: The number of consecutive registers to read. 1 register = 16 bits (2 bytes). For integers, each register is one number. For text data, specify enough registers to hold the text (1 register = 2 bytes).
- **Slave Identifier**: The Modbus slave ID of the target device (1-255). The default is 1. If you have multiple devices on the same bus, each must have a unique slave ID.
...
1. Click in the appropriate value column (Numeric, Text, or Bits).

1. Enter the new value in the same format as displayed.

1. Press Enter to send the write command to the device.
   ...
   Upon a successful write, the Timestamp column updates to reflect the write operation.

### Configuration Page

You can make changes to the [Register Configuration table](#register-configuration) in two ways:

- **Use the context menu**: Right-click anywhere in the table to add a new row, delete selected rows, or clear the entire table.
- **Modify the existing configuration**: Click cells to edit parameters. Changes are applied immediately and trigger a new poll.

> [!IMPORTANT]
> Changes to the configuration table trigger the immediate polling of the affected register. The timer continues to poll all configured registers at the specified interval (default: 30 seconds).

### Polling Behavior

The connector iterates through all rows in the Register Configuration table. Each register is polled sequentially.

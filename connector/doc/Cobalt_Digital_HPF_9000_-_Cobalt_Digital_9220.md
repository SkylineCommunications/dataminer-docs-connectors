...
This dynamic virtual element is created by toggling DVE creation for a 9220 card in a slot on the parent connector. Its pages — **General**, **Status**, **Ports**, **IP**, **Connectivity**, and **Licensing** — display the card's product information, network and ASI port configuration, IP input/output, connection status, and licensing. The element updates as long as the card's display remains enabled on the parent's Card Display page.

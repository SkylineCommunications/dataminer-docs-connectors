...
SolarEdge is an open API, so the connector will begin fetching data from the moment an Account Key is provided. The Account key can be inserted on the **Element Settings** page.
...
Once an element is configured, the connector automatically starts polling data from the SolarEdge API, retrieving current sites linked, inverter real-time data, and meter real-time data for all linked sites.

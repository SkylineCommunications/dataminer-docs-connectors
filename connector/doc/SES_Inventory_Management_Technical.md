...
The SES Inventory Management connector is an HTTP-based DataMiner connector that integrates with the SNOW system and interfaces with the TMF 638 (Service Inventory Management) and TMF 639 (Resource Inventory Management) APIs. It retrieves and monitors customer-facing services, carrier elements, and network resources, storing all data in **DataMiner Object Model (DOM)** instances for structured data management.
...
- **Periodic cache updates**: After initial buildup, the connector periodically polls for items updated in the last 24 hours (`updatedDaysAgo=0`) based on the configured Cache Update Frequency. For services, cleanup of non-operational services runs on every poll after the initial cache buildup completes.
...
- **Deduplication**: Removes duplicate ExternalIds from API responses before processing.
- **Status Tracking**: Maps operational status from `service.Status` API field.
- **Cleanup Logic**: Removes services where operational status is not "Operational" on every poll after initial cache buildup.
- **Field Mapping**:
...
- **Deduplication**: Filters out carriers with null/empty ExternalIds and removes duplicates.
...
- **Type Filtering**: Excludes the following resource types before DOM processing:
  - Card, BandwidthSegment, LogicalPort, Multicast, Slot, Port, Circuit
...
2. **ID Tracking**: Entity IDs are accumulated across pages in a dedicated parameter:
   - Services: `Parameter.servicesids_4998`
   - Carriers: `Parameter.carriersids_2998`
   - Resources: `Parameter.resourcesids_3998`
3. **Continuation**: If page is full (`services.Length == Global.RecordsPerPage`), trigger next page.
4. **Final Page**: When partial page is received:
   - Perform cleanup of deprecated entities (except Services)
   - Clear cache buildup flag
   - Clear ID tracking parameter
...
1. **Create or Update**: Incoming API data is compared against existing DOM instances by ExternalId:
   - If instance exists: Updates all fields via mapper
   - If new: Creates new DOM instance with mapped fields
   
2. **Cleanup**: Deprecated entities (not in current API response) are removed:
   - Carriers/Resources: Only during final cache buildup page
   - Services: On every poll after cache buildup completes

### Error Handling

- **Null/Empty IDs**: Entities with null or empty ExternalIds are skipped with logging.
- **Duplicate IDs**: First occurrence is kept, duplicates are logged and skipped.
...
- **DOM module** must be available and accessible for the connector to create definitions and instances.
...
- Duplicate ExternalIds are automatically handled - first occurrence is kept, subsequent duplicates are skipped.
- Modem status is preserved during resource updates to maintain operational continuity.
- Services cleanup runs on every poll after cache buildup, while Carriers and Resources cleanup only during cache buildup.
...
1. **File Handling**: Enable this toggle to activate the file provisioning logic.

2. **Provisioning Interval**: Set the interval (in seconds) for how often the provisioning logic should run. The default is 20 seconds, with a range of 10 seconds to 1 day.

3. **Mapping File Import Path**: Specify the path to the JSON mapping instruction file that defines how source data fields are transformed to output fields.

4. **User Name** and **Password**: If using remote file locations, provide the credentials for network share access. The username can be in the format `DOMAIN\username` or just `username`.
...
2. Scans the connector import path for CC files (XML or CSV).
3. For each CC file, filters inventory records by matching circuit aliases.
4. Applies the mapping rules to transform data fields.
5. Generates output JSON files with a `_CP` suffix in the export folder.
...
The connector validates the mapping file. These rules must be met:

1.  `emitArray` must be a non-empty string
2.  `fields` must be present (can be empty)
3.  `arrays` must be present (can be empty)
4.  Each field rule must have a `to` property
5.  Each field rule must have either `from` OR `const` (not both)
6.  Each array rule must have `to`, `from`, and `itemFields`

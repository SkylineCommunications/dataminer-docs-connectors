...
### Scheduler Events
Introduced in version 1.1.6.22, this page allows to monitor and control scheduled events created under each channel. To add events (on the **Add Events** sub-page), first the user needs to select and set the channel (**Input Channel**). Then the connector will retrieve the available profiles of that channel and make them available for selection in the **Channel Profile** field drop down list.

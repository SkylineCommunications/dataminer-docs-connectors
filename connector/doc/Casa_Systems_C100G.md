...
For the edge-QAM side of the platform, the connector surfaces video QAM port, channel, and input port statistics, together with detailed video session KPIs such as bit rates and transport stream health. This allows operators to ensure reliable video service delivery and detect degradations before they reach subscribers.
...
For detailed information on connections, configuration, and usage of each range, see the [technical documentation](xref:Connector_help_Casa_Systems_C100G_Technical).

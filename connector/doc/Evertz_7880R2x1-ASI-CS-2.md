...
- **Switch Method**: Sets the mode of the switching depending on the type of the input streams. The following settings are possible:
  ...
  > [!NOTE]
  > Changing this parameter while the switch is operating may result in the switch being reset. If the parameter is switched to *packetSwitch* and then back to *gopBoundary* or *packetMatching*, the switch will be reset.
...
- **Reset Switch**: Allows you to reset the switch state.
  ...
  > [!IMPORTANT]
  > When the switch is reset, the output will temporarily drop due to the switch resynchronizing process.

...
- **Monitor active path selection**: Track whether the main or backup input is currently selected for each protected service.
- **Follow redundancy health**: Observe synchronization status, bitrate information, and switching behavior to detect instability before it impacts viewers.
- **Surface transport stream issues**: Keep an eye on broadcast-specific indicators such as ETR 290-related errors.
- **Review switching history**: Use switch counters and alarm logs to understand recurring failover events and recurring network problems.
...
> [!NOTE]
> For detailed device behavior, configuration options, and low-level parameter information, refer to the official Enensys IPGuard v3 product documentation.
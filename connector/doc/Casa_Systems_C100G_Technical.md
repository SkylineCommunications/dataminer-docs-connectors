...
This connector monitors and controls the Casa Systems C100G Converged Cable Access Platform (CCAP) over SNMPv2. Over time, the connector has evolved into several ranges, each focusing on a specific set of capabilities. Because the ranges differ significantly in scope and layout, their usage is documented per range below.
...
This is a customer-specific range with a redesigned layout. Later minor versions in this range added **DCF (DataMiner Connectivity Framework)** support and **LLDP** parameters.

> [!NOTE]
> When you use this range, set the timeout of the element to 10 seconds to avoid unnecessary timeouts. If a restart is required, stop the element and wait a few seconds before restarting it, so the equipment can finish any previously requested actions.
...
DCF is supported starting from range 5.0.1.x (version 5.0.1.1). In the other ranges, this connector does not support the DataMiner Connectivity Framework (DCF).

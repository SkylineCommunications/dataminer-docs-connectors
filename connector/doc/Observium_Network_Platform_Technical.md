...
> [!IMPORTANT]
> The Observium REST API is only included in the paid Subscription Editions (Professional or Enterprise) of Observium. The free Community Edition does not include the REST API and therefore cannot be integrated with this connector.
...
- **IP address/host**: The polling IP address or URL of the Observium web server. When the Observium server is only reachable over HTTPS, prefix the address with *https://*.
...
1. Select the desired **Authentication Mode**:
   ...
   - *API Token*: Authentication using an Observium API token, sent in the standard *Authorization* HTTP header (Bearer scheme).

1. Enter the credentials corresponding to the selected mode.

To retrieve the complete device inventory, the configured account (or the account backing the API token) must have at least Observium user level 5 (*Global Read*). Accounts with a lower level only return the devices explicitly permitted to them.

Polling will fail with a clear status message until credentials have been configured for the selected authentication mode.

## How to Use

The connector retrieves data by polling the Observium REST API (`/api/v0`) over HTTP(S). Because the Observium API does not support push notifications, all data is polled. Data freshness is therefore bounded by the configured polling intervals.
...
Polling is controlled through the **Polling Manager**, available via a page button on the **Configuration** page. Each poll type has a row showing its **State** (Enabled/Disabled), **Last Poll** timestamp, **Interval**, **Status**, and **Details**. Polls can also be triggered on demand using the **Poll** button, including for disabled rows, allowing manual overrides.

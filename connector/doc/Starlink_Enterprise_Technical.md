...
Because the Starlink Enterprise element is relying on a Starlink Enterprise Account element to send the Opt In/Out request to the Starlink API, it may take a minute or two for the **Automatic Top Up** togglebutton to update after pressing it. This is due to the [buffer mechanism](#initialization) which was added to allow inter-element communication. Please keep these background actions in mind when toggling that button.

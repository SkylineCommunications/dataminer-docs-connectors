...
- **Telemetry Linger Duration** represents the maximum number of milliseconds to collect telemetry entries. The recommended linger duration is ~15000 ms. This duration is recommended by Starlink in order for all the data points to come in accurately. If the telemetry requests are timing out, please check the **Timeout of a single command (ms)** setting in the element configuration wizard. Preferably, use a Timeout of a single command which is slightly bigger than the Telemetry Linger Duration.

> [!CAUTION]
> A Telemetry Linger Duration of 15 000 ms only allows 4 accounts to be polled at maximum. If you want to poll more accounts, please lower the Telemetry Linger Duration but make sure to meet the following subtraction:

...
The **Skyline EPM Platform GPON WM** connector monitors and manages distributed workflow execution for the **DataMiner EPM xPON** Solution through a virtual connection.
...
1. Configure shared settings (available on both **Master Configuration** and **Slave Configuration**):
   ...
   - **System Credentials**: Username and password.
   - **Import Settings**:
...
1. Configure role-specific settings:
   ...
   - **Slave role**:
      - **Master Name** and **Master ID** (displayed on the **Slave Configuration** page).
...
On this page, you need to set **Element Role** to define Master or Slave behavior.

#### Workflows Page (Slave)

- Displays the **Workflow Overview** queue and status tracking.
- Tracks the workflow lifecycle.
...
This page displays the **Slave Overview** with registered slave elements, processing status, and last usage time.

#### InterApp Messages Page

This page displays inbound and outbound InterApp traffic and response status information for distributed processing visibility.
...
1. Requests are received through InterApp messaging and queued in the workflow table.

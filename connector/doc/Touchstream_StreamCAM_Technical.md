...
Dynamic streams are how event-based monitoring is provisioned. Instead of configuring the streams for each event by hand, you define a reusable **template** describing the streams an event of that type needs. When an event is received, the connector expands the assigned template into concrete streams and provisions them in StreamCAM.
...
2. The connector looks up the template name(s) on that row in the **Event Template** table.
3. Every matching template row is expanded into one stream, grouped per dynamic group.
4. The resulting streams are sent to StreamCAM in a bulk call per dynamic group, and start being monitored.
5. When the event ends, the streams are deactivated and eventually archived or deleted.

#### Dynamic Streams Template

The **Dynamic Streams Template** page holds the **Event Template** table. Rows are keyed by template name, and a template consists of *all* rows sharing the same **Template Name** — one row per stream that the template should produce. Each row defines the **Product**, **Format**, **CDN**, **Channel**, **Manifest**, **Headers**, **Environment**, the Adobe equivalents, and the **Dynamic Group** the resulting stream belongs to.
...
- **Delete Archived Streams Automatically**: When enabled, archived streams are deleted as well.

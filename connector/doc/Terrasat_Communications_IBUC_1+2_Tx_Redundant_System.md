...
- **Per-BUC insight**: Exposes dedicated info, alarm and sensor tables for each BUC (A, B and Spare), including power, frequency band, temperature and input/output power levels.
...
- **Sensor readings**: Reports supply voltages, 10 MHz level and per-BUC sensor values (supply voltage/current, DRO voltage, temperature and power levels).
...
**Solution**: Per-BUC sensor and alarm tables are monitored in real time, with alarming on thresholds such as temperature, input/output power and supply values.
...
**Benefit**: A single pane of glass for the uplink infrastructure, simplifying operations and troubleshooting.
...
- **SNMP community strings** are needed to read from (and, where applicable, write to) the device.

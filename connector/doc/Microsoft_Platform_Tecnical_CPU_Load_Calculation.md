...
Timestamp value in 100 nanoseconds units. Available in `Win32_PerfRawData_PerfOS_Processor`.
...
- It displays the average percentage of busy time observed during the sample interval.
...
Since the performance counter is based on **Idle Time**, not directly on busy time. Windows tracks how long the processor spends in the **Idle Thread** as follows:

- Raw counter values ($N_{1}$, $N_{2}$) represents cumulative idle time ticks at two sampling points of `Percent Processor Time`.

$$
\Delta\,\,\text{Percentage Processor Time}= N_{2}-N_{1}
$$

- Raw base values ($D_{1}$, $D_{2}$) represents cumulative total ticks at those points (from `Timestamp_Sys100NS`)

$$
\Delta\,\,\text{Timeticks} = D_{2}-D_{1}
$$

Since the counter type identifies it as inverse, the following formula is used:

Based on the following Microsoft Reference [Link](https://learn.microsoft.com/en-us/windows/win32/wmisdk/monitoring-performance-data#using-raw-performance-data-classes), the CPU load should be calculated as follows:
...
- The `Percentage Processor Time` is a time-based metric computed from idle vs. busy time over a sampling interval. They do not depend on the CPU's clock frequency.

- The metric `Percentage Processor Time` is capped at 100%.

- The use `Current Speed` and `Base Speed` can lead to CPU usage higher than 100%. Reference: [Link](https://learn.microsoft.com/en-us/troubleshoot/windows-client/performance/cpu-usage-exceeds-**100**)

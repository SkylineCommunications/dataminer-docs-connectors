...
If the name of the Starlink Enterprise Account element isn't fully matching the account name as shown on the Starlink web interface, the parameter set logic is not going to work.

A heartbeat check was implemented before querying the Starlink API to confirm that the Starlink Enterprise element is able to receive the response content. If the Starlink Enterprise element is busy, the API request is postponed.
...
To see the actual traffic between the Starlink Enterpise Account element and the Starlink Enterprise element, please check the element log file. You can open the element log file by right-clicking the element in the Surveyor and selecting **View** > **Log**.

To see the actual traffic between the Starlink Enterpise Account element and the Starlink API, a built-in DataMiner tool called Stream Viewer can be used. You can access it by right-clicking the element in the Surveyor and selecting **View** > **Stream Viewer**.
...
The **Starlink Enterprise Element Name** parameter defaults to *Starlink Enterprise*. If the name of your Starlink Enterprise element is different, please change this parameter to the correct element name.

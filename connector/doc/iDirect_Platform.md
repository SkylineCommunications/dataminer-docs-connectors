...
| 10.0.7.x [SLC Main] | Removed DisplayColumn feature.<br>Splitted DVE creation and polling, so it's now possible to poll all the devices without creation DVEs for all of them. | 10.0.6.21 | - **Lose all trend and alarm history data**<br>- Backend cleanup but can be broken with old lingering data |

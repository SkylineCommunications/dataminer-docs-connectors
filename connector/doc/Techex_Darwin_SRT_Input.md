...
- **Module information**: Retrieve and monitor general information about the SRT Input modules, including network and transport configuration, and FEC configuration.
- **Real-time statistics**: View statistics updated in real time for each subscribed module thanks to Kafka subscriptions. These include stream, traffic, and latency statistics for each module.
- **Reliability and recovery monitoring**: Retrieve data from each SRT Input Module to keep an eye on the network status and recovery.

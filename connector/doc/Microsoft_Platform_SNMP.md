...
- In order to configure correctly the SNMP service on devices running Windows Server OS, the following features must be enabled:
  - SNMP Service
  - Remote Server Administration Tools -> Feature Administration Tools -> SNMP Tools. This last feature allows to perform additional settings in the SNMP agent (e.g. set the community string, define hosts that can monitor the server, etc.)

More information about how to enable features on Windows Server can be found in [Microsoft Learn](https://learn.microsoft.com/en-us/windows-server/administration/server-manager/install-or-uninstall-roles-role-services-or-features#install-roles-role-services-and-features-by-using-the-add-roles-and-features-wizard).

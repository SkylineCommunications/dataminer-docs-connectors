...
Open-Meteo is a free and open API, so the connector will begin fetching weather data without any additional credentials. However, users with a commercial license can provide an API key on the **Element Settings**.
...
- The **Locations** page allows adding new locations by specifying the coordinates for each site. The connector polls weather data individually for every configured location.  
- On the **API Configuration** page, users can adjust polling parameters and select which weather variables to retrieve from the API.